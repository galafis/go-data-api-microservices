...
// jwtServiceImpl is the concrete implementation of JWTService interface
...
func (s *jwtServiceImpl) GenerateAccessToken(user *models.User) (string, error) {
...
func (s *jwtServiceImpl) GenerateRefreshToken(user *models.User) (string, error) {
...
func (s *jwtServiceImpl) ExtractUserID(tokenString string) (uuid.UUID, error) {
...
func (s *jwtServiceImpl) ExtractTokenType(tokenString string) (string, error) {
	claims, err := s.ValidateToken(tokenString)
	if err != nil {
		return "", err
	}
...
func (s *jwtServiceImpl) GetTokenExpiry(tokenType string) time.Duration {
	switch tokenType {
	case "access":
